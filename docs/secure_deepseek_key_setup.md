...
> 项目没有把 key 写入配置文件或日志，而是通过本机环境变量 / `.env` 注入。CLI 的 `deepseek-check` 只验证配置状态，`deepseek-smoke` 负责一次真实 API 连通性验证，且必须显式加 `--allow-llm-calls`，所有 run artifacts 只保存 usage、trace、tool calls 和 patch，不保存密钥。
